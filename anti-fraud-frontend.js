...
(function (global) {
  ...
  async function collect() {
    ...
    _pending = (async function () {
      ...
      var audio = await getAudioFingerprint();
      parts.push(audio);

      var rawSignature = parts.join('##');
      var fingerprint = await sha256Hex(rawSignature);
      var deviceId = await getOrCreateDeviceId();
      var suspiciousFlags = getSuspiciousFlags();

      _cache = { fingerprint: fingerprint, deviceId: deviceId, suspiciousFlags: suspiciousFlags };
      return _cache;
    })();
